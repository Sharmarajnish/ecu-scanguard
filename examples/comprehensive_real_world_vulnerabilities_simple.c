...
void process_diagnostic_message_can(void* msg);
...
void send_pid_response_obd(uint8_t pid, uint8_t* data);
void request_diagnostic_data_obd(uint8_t pid);
...
void process_can_message(can_message_t* msg) {
    ...
    if (msg->id >= 0x7E0 && msg->id <= 0x7E7) {  // UDS diagnostic range
        process_diagnostic_message_can(msg);
    }
}

// VULNERABLE: Buffer overflow in diagnostic processing
void process_diagnostic_message_can(can_message_t* msg) {
    char buffer[64];
...
const char* bmw_diagnostic_key = "bmw_diag_2021";
...
const char* vw_service_key = "vw_service_mode";
...
int main() {
    ...
    printf("- Buffer overflows (CWE-787)\n");
    printf("- Command injection (CWE-78)\n");
    printf("- Hardcoded credentials (CWE-798)\n");
    printf("- Format string vulnerabilities (CWE-134)\n");
    printf("- SQL injection (CWE-89)\n");
    printf("- Weak authentication (CWE-287)\n");
    printf("- Timing attacks (CWE-208)\n");
    printf("- Predictable random numbers (CWE-330)\n");
    printf("- No rate limiting (CWE-770)\n");
    printf("- Weak encryption (CWE-327)\n");
    printf("- No input validation (CWE-20)\n");
    printf("- Stack overflows (CWE-787)\n");
    printf("- No access control (CWE-284)\n");
    printf("- No signature verification (CWE-347)\n");
    printf("- No bounds checking (CWE-787)\n");
...
void process_diagnostic_message_can(void* msg) { printf("Diagnostic message processed\n"); }
...
void send_pid_response_obd(uint8_t pid, uint8_t* data) { printf("PID response sent: 0x%02X\n", pid); }
void request_diagnostic_data_obd(uint8_t pid) { printf("OBD diagnostic request: 0x%02X\n", pid); }
